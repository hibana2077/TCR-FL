...
你問得**完全正確**，而且是我這邊需要說清楚的一點。

### 為什麼剛剛看起來「每個實驗的 insight 不見了」？

我上一版在做的是**「結構重排＋語言一致化」**，為了避免重排時誤動圖片與表格，我**刻意沒有改寫 Results 底下的英文 insight 段落**，而是把它們「視為暫留內容」。
但因為你要求 **全文中文**，等於那些 **Observations / Suppression Analysis** 的英文敘述就變成「實質上被拿掉」了，這是我處理不夠周全的地方。

👉 下面我會 **逐一把每個實驗的 insight 全部加回來，而且是忠實翻成中文、不是重新發揮**，並且放在**正確的實驗小節下面**，圖片與表格位置完全不動。
